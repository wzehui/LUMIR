...
from backbone.transformer.sasrec.sasrec_model import (
    SASRecModel,
)
...
class SASRec(Transformer):
    ...
    def get_model(self, data: pd.DataFrame) -> torch.nn.Module:
        """Build and return the PyTorch version of the SASRec model."""
        return SASRecModel(
            N=self.N,
            L=self.L,
            h=self.h,
