...
from backbone.data.abstract_dataset import Dataset
from backbone.eval import metrics
...
class EvaluationReport:
    ...
    sample_ids: Optional[np.ndarray] = None  # NEW: keep sample ids order-aligned
    ...
    def to_per_sample_df(self, allowlist: Optional[List[str]] = None,
                         strict: bool = False) -> pd.DataFrame:
        """
        Convert this EvaluationReport to a per-sample DataFrame safely.

        Each row corresponds to one test instance (e.g., a user/session).
        Columns:
            - sample_id
            - One column per metric that truly has per-sample values
            - '__model__', '__topk__'

        Parameters
        ----------
        allowlist : list[str], optional
            Only include metric names in this list. (e.g., ["NDCG@10","NDCG@20"])
            If None, include all valid metrics with correct per-sample length.
        strict : bool, default False
            If True, raise errors for mismatched metrics.
            If False, silently skip metrics with wrong length or empty arrays.
        """
        # --- Sanity checks ---
        if not self.results_per_sample:
            raise ValueError(
                "No per-sample metrics available. Run eval with metrics_per_sample=True.")
        if self.sample_ids is None:
            raise ValueError(
                "sample_ids missing; eval did not attach sample_ids.")

        n = len(self.sample_ids)
        df = pd.DataFrame({"sample_id": self.sample_ids})
        added = []  # Track which metrics were added

        # --- Iterate over all available metrics ---
        for metric_name, arr in self.results_per_sample.items():
            # Skip if allowlist specified and metric not included
            if allowlist is not None and metric_name not in allowlist:
                continue

            arr = np.asarray(arr)

            # Skip empty, scalar, or None metrics
            if arr.ndim == 0 or arr.size == 0:
                if strict:
                    raise ValueError(
                        f"{metric_name} has empty/non-vector per-sample output.")
                else:
                    # This metric has no per-sample output (e.g., Catalog coverage)
                    continue
            ...
            if len(arr) != n:
                if strict:
                    raise ValueError(
                        f"Length mismatch for {metric_name}: {len(arr)} vs {n}")
                else:
                    # Skip metrics whose length doesn't match number of samples
                    continue

            # Add this metric column
            df[metric_name] = arr
            added.append(metric_name)

        # --- Check if any valid metric was added ---
        if not added and strict:
            raise ValueError("No valid per-sample metrics found (all skipped).")

        # --- Add metadata columns ---
        df["__model__"] = self.model_name
        df["__topk__"] = self.top_k
...
class Evaluation:
        ...
        if self.use_folds:
            ...
            for model in self.models:
                ...
                for i, (train_data, test_data_labels, test_data) in enumerate(
                    ...
                    # # Mask out the mask token from appearing in the Top-K ranking
                    # mask_token_id = model.mask_target_used if hasattr(model, "mask_target_used") else None
                    # if mask_token_id is not None:
                    #     for rec in recommendations.values():
                    #         rec[rec == mask_token_id] = -np.inf
    ...
    def eval(
        ...
        if cores == 1:
            for metric in metrics:
                ...
                if metrics_per_sample:
                    ...
                    eval_report.results_per_sample[metric.name()] = np.array(metric_result_bulk)

                metric_result = metric.state_merge(metric_state, metric_result)
                eval_report.results[metric.name()] = metric.state_finalize(metric_result)

                logger.info(f"[EVALUATION] Computing {metric.name()} completed.")

            if metrics_per_sample:
                eval_report.sample_ids = sample_ids  # NEW: attach sample ids
        ...
        slices = []  # NEW: record the slices to rebuild sample_ids order

        for i in range(cores):
            size = q + (i < r)
            end = start + size
            slices.append((start, end))  # NEW
            core_results.append(
                pool.apply_async(
        ...
        # Prepare a container for per-sample concat in the same slice order
        sample_ids_concat = []  # NEW: hold concatenated ids in the same order as metric_all_sample
        ...
        for i, metric in enumerate(metrics):
            ...
            # 拉平所有 core 的指标
            for (s, e), all_metric_results in zip(slices, core_results):  # NEW: use slices
            ...
            if metrics_per_sample and len(metric_all_sample) > 0:
                # metric_all_sample 是各核的数组（和 slices 对齐），直接按顺序拼接
                eval_report.results_per_sample[metric.name()] = np.concatenate(metric_all_sample)

        if metrics_per_sample:
            # 按 slices 顺序拼接 sample_ids，确保与 metric_all_sample 的拼接顺序一致
            for (s, e) in slices:
                sample_ids_concat.append(sample_ids[s:e])
            eval_report.sample_ids = np.concatenate(sample_ids_concat)  # NEW

        logger.info(
            f"[EVALUATION] Computing {', '.join([m.name() for m in metrics])} completed."
        )
