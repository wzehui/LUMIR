...
SEARCH_SPACE = {
    # loss weights
    "lambda_modality": [1.0],
    "lambda_align": [0.1, 0.2, 0.35, 0.5, 0.7, 1.0],
    # "lambda_align": [0],
    ...
    # LAlign
    "tau_align": [0.03, 0.05, 0.07, 0.1, 0.15, 0.2],
...
RESULTS_CSV = "./hp_search_nn_hit_mrr.csv"
...
def build_losses(config: Dict[str, Any], dataset: MultiViewPOIDataset):
    ...
    seqalign_loss_fn = MemoryBankAlignLoss(
        ...
        temperature=float(config["loss"]["tau_align"]),
        neg_sample_size=1000,
        pos_window=int(config["loss"]["pos_window"]),
        pos_strategy=str(config["loss"]["pos_strategy"]),
        normalize_store=True,
        update_bank=True
    )
    return comm_loss_fn, seqalign_loss_fn, align_memory_bank


@torch.no_grad()
def warmup_align_memory_bank(
    dataloader: DataLoader,
    encoder: MultisourceTextFusionEncoder,
    fusion_module: CoMMFusionModule,
    align_memory_bank: Dict[Any, torch.Tensor],
    ...
    for batch in tqdm(dataloader, desc="Warmup AlignBank", leave=False):
...
def train_one_run_and_eval(
    ...
    config["loss"]["lambda_align"] = float(hp["lambda_align"])
    config["loss"]["tau_modality"] = float(hp["tau_modality"])
    config["loss"]["tau_align"] = float(hp["tau_align"])
    ...
    config["loss"]["pos_strategy"] = str(hp["pos_strategy"])
    config["loss"]["pos_window"] = int(hp["pos_window"])
    ...
    comm_loss_fn, seqalign_loss_fn, align_memory_bank = build_losses(config, dataset)
...
def main():
    ...
    pruned = []
    for hp in combos:
        if hp["pos_strategy"] != "window" and hp["pos_window"] != SEARCH_SPACE["pos_window"][0]:
            continue
        pruned.append(hp)

    combos = sample_combos(pruned, MAX_TRIALS, seed=GLOBAL_SEED)
